...
### 1.1. Фактическое состояние реализации (актуально на 2026-06-05)

Документ изначально предполагал канал **GSConnect/KDE Connect**. В ходе разработки
от него **отказались** (конфликт за TCP-порт 1716 с официальным клиентом KDE Connect),
и реализован **собственный канал**. Ниже — фактически сделанное; разделы 6 и далее
отражают исходный замысел и сохранены как история проектных решений.

- **Канал связи:** собственный **TCP-протокол** (построчный JSON, порт `5390`,
  предварительный токен). ПК-агент — Python-сервер `ubuntu-agent/rfid-server.py`
  (systemd user-сервис); Android — клиент `TcpCommandClient`. GSConnect/KDE Connect
  **не используется**.
- **Профили ПК и QR:** ПК добавляется сканированием **QR-кода** из иконки агента в
  системном **трее** (`rfid-tray.py`). Главный экран приложения — **сетка плиток** ПК
  со статусом замка и иконкой ОС; клик переключает LOCK/UNLOCK, долгое нажатие —
  сведения/удаление.
- **Привязка меток:** метка привязывается к профилю ПК либо «универсальная» (открывает
  экран плиток без команд).
- **Дополнительный триггер:** **LOCK при отключении телефона от зарядки** (снятие с
  Type-C дока) — в дополнение к снятию NFC-метки.
- **Сетенезависимость (в проработке):** для работы вне общей локальной сети
  проектируется отдельный сетевой слой на базе **приватного Yggdrasil** (P2P mesh с
  E2E-шифрованием) — см. `Архитектура-сетенезависимая-связь.md`.
- **Публикация:** проект готовится к публичному репозиторию под лицензией **MIT**
  (`LICENSE`, `README.md`); секреты и реальные адреса инфраструктуры в репозиторий не
  попадают.
...
- **US-18.** Как пользователь, я хочу добавить в **шторку быстрых настроек** плитку
  (одну или несколько), каждая из которых управляет **своим ПК**: тап — переключение
  блокировки, без открытия приложения.
...
- **FR-A27.** Приложение предоставляет **плитки быстрых настроек** (TileService,
  4 слота): каждой плитке назначается профиль ПК (тап по ненастроенной плитке или
  долгое нажатие — выбор ПК); тап по настроенной плитке переключает блокировку;
  плитка отражает статус (активна = разблокирован, подпись: заблокирован/офлайн).
...
**Реализовано (2026-08-29):** команды подписываются
`sig = HMAC-SHA256(token, "cmd|reqId|ts")`; токен по сети не передаётся.
Сервер проверяет окно времени (±300 с) и уникальность nonce `reqId`
(anti-replay, кэш в памяти); все отказы — единый ответ `unauthorized`
(причина только в логе). Регрессионный тест: `ubuntu-agent/test_auth.py`.
Требует ревью человеком (криптография, A13/7.3).

Не реализовано (при необходимости): привязка к идентификатору устройства и
метке; шифрование полезной нагрузки (сейчас конфиденциальность обеспечивает
транспорт — ZeroTier E2E).
