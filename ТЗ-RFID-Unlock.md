...
- **US-12.** Как пользователь, я хочу **вручную заблокировать или разблокировать ПК**
  кнопками в приложении (UNLOCK/LOCK), не прибегая к метке.
...
- **FR-A18.** В верхней части главного экрана — две кнопки ручного управления:
  **UNLOCK** (слева, пиктограмма открытого замка) и **LOCK** (справа, пиктограмма
  закрытого замка), отправляющие команды UNLOCK/LOCK на выбранный ПК напрямую,
  без участия NFC-метки.
