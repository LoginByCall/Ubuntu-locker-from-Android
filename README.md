...
```
sudo: apt-get update
Подтвердите на телефоне или нажмите любую клавишу, чтобы ввести пароль:
```

Нажали кнопку на телефоне — `sudo` пускает, пароль не спрашивается. Нажали
любую клавишу — запрос с телефона снимается и открывается обычное приглашение
пароля. Отказ и таймаут ведут туда же: ослабить `sudo` эта строка не может.
